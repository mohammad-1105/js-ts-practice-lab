...
import { generateStudentReport } from "../src/07-student-report-card.ts";

describe("07 - Student Report Card Generator (9 pts)", () => {
  describe("Grade calculation", () => {
    test("Percentage >= 90 gives A+", () => {
      const result = generateStudentReport({
        name: "Topper",
        marks: { maths: 95, science: 92, english: 96 },
      });
    ...
    test("Percentage >= 80 but < 90 gives A", () => {
      const result = generateStudentReport({
        name: "Good",
        marks: { maths: 85, science: 82, english: 80 },
      });
      expect(result.grade).toBe("A");
    });
    ...
    test("Percentage >= 70 but < 80 gives B", () => {
      const result = generateStudentReport({
        name: "Decent",
        marks: { maths: 75, science: 72, english: 70 },
      });
    ...
    test("Percentage >= 60 but < 70 gives C", () => {
      const result = generateStudentReport({
        name: "Average",
        marks: { maths: 65, science: 62, english: 60 },
      });
    ...
    test("Percentage >= 40 but < 60 gives D", () => {
      const result = generateStudentReport({
        name: "Below",
        marks: { maths: 45, science: 42, english: 50 },
      });
    ...
    test("Percentage < 40 gives F", () => {
      const result = generateStudentReport({
        name: "Fail",
        marks: { maths: 35, science: 28, english: 30 },
      });
  ...
  describe("Statistics calculation", () => {
    ...
    test("percentage with decimals", () => {
      const result = generateStudentReport({
        name: "Taylor",
        marks: { maths: 73, science: 67, english: 81 },
      });
    ...
    test("name is preserved", () => {
      const result = generateStudentReport({ name: "Alex", marks: { maths: 85 } });
  ...
  describe("Pass/Fail subjects", () => {
    test("All passed subjects (>= 40)", () => {
      const result = generateStudentReport({ name: "Alex", marks: { maths: 85, science: 92 } });
      expect(result.passedSubjects).toEqual(["maths", "science"]);
      expect(result.failedSubjects).toEqual([]);
    });

    test("All failed subjects (< 40)", () => {
      const result = generateStudentReport({ name: "Fail", marks: { maths: 20, science: 15 } });
    ...
    test("Mix of passed and failed", () => {
      const result = generateStudentReport({
        name: "Mix",
        marks: { maths: 85, science: 30, english: 55 },
      });
    ...
    test("Exactly 40 marks counts as passed", () => {
      const result = generateStudentReport({ name: "Edge", marks: { maths: 40 } });
      expect(result.passedSubjects).toContain("maths");
  ...
  describe("Edge cases", () => {
    test("Single subject student", () => {
      const result = generateStudentReport({ name: "Solo", marks: { maths: 75 } });
  ...
  describe("Validation", () => {
    test("null student returns null", () => {
      expect(generateStudentReport(null as any)).toBeNull();
    });

    test("Missing name returns null", () => {
      expect(generateStudentReport({ marks: { maths: 85 } })).toBeNull();
    });

    test("Empty name returns null", () => {
      expect(generateStudentReport({ name: "", marks: { maths: 85 } })).toBeNull();
    });

    test("Missing marks returns null", () => {
      expect(generateStudentReport({ name: "Alex" })).toBeNull();
    });

    test("Empty marks object returns null", () => {
      expect(generateStudentReport({ name: "Alex", marks: {} })).toBeNull();
    });

    test("Mark out of range (> 100) returns null", () => {
      expect(generateStudentReport({ name: "Alex", marks: { maths: 150 } })).toBeNull();
    });

    test("Negative mark returns null", () => {
      expect(generateStudentReport({ name: "Alex", marks: { maths: -10 } })).toBeNull();
    });

    test("Non-number mark returns null", () => {
      expect(generateStudentReport({ name: "Alex", marks: { maths: "eighty" } })).toBeNull();
    });

    test("Non-object input returns null", () => {
      expect(generateStudentReport("student")).toBeNull();
    });
  });
});
