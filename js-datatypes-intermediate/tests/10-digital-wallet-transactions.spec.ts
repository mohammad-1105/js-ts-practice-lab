...
import { analyzeWalletTransactions } from "../src/10-digital-wallet-transactions.ts";

describe("10 - Digital Wallet Transaction Analyzer (9 pts)", () => {
  const sampleTransactions = [
    ...
    { id: "T2", type: "debit", amount: 200, to: "DoorDash", category: "food", date: "2025-01-02" },
    { id: "T3", type: "debit", amount: 100, to: "DoorDash", category: "food", date: "2025-01-03" },
    { id: "T4", type: "debit", amount: 1500, to: "Rent", category: "housing", date: "2025-01-05" },
    { id: "T5", type: "credit", amount: 300, to: "Alex", category: "refund", date: "2025-01-06" },
  ];

  describe("Credit/Debit calculations", () => {
    test("totalCredit sums only credit transactions", () => {
      const result = analyzeWalletTransactions(sampleTransactions);
    ...
    test("netBalance can be negative", () => {
      const result = analyzeWalletTransactions([
        {
          id: "T1",
          type: "debit",
          amount: 5000,
  ...
  describe("Frequent contact", () => {
    test("Most frequent contact returned", () => {
      const result = analyzeWalletTransactions(sampleTransactions);
      expect(result.frequentContact).toBe("DoorDash"); // appears 2 times
    });
    ...
    test("Single transaction - that contact is frequent", () => {
      ...
      expect(result.frequentContact).toBe("UberEats");
    });
  });
  ...
  describe("Validation", () => {
    test("Empty array returns null", () => {
      expect(analyzeWalletTransactions([])).toBeNull();
    });

    test("Non-array returns null", () => {
      expect(analyzeWalletTransactions("transactions")).toBeNull();
    });

    test("null returns null", () => {
      expect(analyzeWalletTransactions(null as any)).toBeNull();
    });
  });
});
