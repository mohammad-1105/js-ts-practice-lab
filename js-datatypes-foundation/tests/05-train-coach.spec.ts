...
describe("05 - Train Coach Finder: Array Search & Check (8 pts)", () => {
  ...
  describe("findPassenger", () => {
    test("finds passenger by exact name", () => {
      expect(findPassenger(passengers, "Rahul")).toEqual({
        name: "Rahul",
        coach: "S5",
        seat: 42,
        status: "confirmed",
    ...
    test("finds passenger case-insensitively", () => {
      ...
      expect(findPassenger(passengers, "PRIYA")).toEqual({
        name: "Priya",
        coach: "S3",
        seat: 15,
        status: "waitlisted",
  ...
  describe("isAnyWaitlisted", () => {
    ...
    test("returns false for non-array", () => {
      ...
      expect(isAnyWaitlisted(null as any)).toBe(false);
    });
  });
  ...
  describe("areAllConfirmed", () => {
    ...
    test("returns false for non-array", () => {
      ...
      expect(areAllConfirmed(null as any)).toBe(false);
    });
  });
  ...
  describe("getWaitlistedPassengers", () => {
    ...
    test("returns [] for non-array", () => {
      ...
      expect(getWaitlistedPassengers(null as any)).toEqual([]);
    });
  });
});
