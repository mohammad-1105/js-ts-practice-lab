...
import {
  ...
  parseFare,
  roundFare,
} from "../src/03-taxi-meter.ts";

describe("03 - Taxi Fare Calculator: Number & Math (8 pts)", () => {
  describe("parseFare", () => {
    ...
    test("returns -1 for non-string input", () => {
      ...
      expect(parseFare(null as any)).toBe(-1);
      expect(parseFare(undefined as any)).toBe(-1);
    });
  });
