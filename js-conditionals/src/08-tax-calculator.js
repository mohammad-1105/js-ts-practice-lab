...
export function calculateTax(income) {
  ...
  if (income > 70_000) {
    // Full tax for lower brackets
    tax += 0.10 * (30_000 - 10_000); // Bracket 2
    tax += 0.20 * (70_000 - 30_000); // Bracket 3

    // Partial tax for top bracket
    tax += 0.30 * (income - 70_000);
  } else if (income > 30_000) {
    // Full tax for bracket 2
    tax += 0.10 * (30_000 - 10_000);

    // Partial tax for bracket 3
    tax += 0.20 * (income - 30_000);
  } else if (income > 10_000) {
    // Partial tax for bracket 2 only
