...
import { calculateCoffeePrice } from "../src/07-coffee-shop.ts";

describe("07 - Bean & Brew: Coffee Order (9 pts)", () => {
  describe("Base prices by size", () => {
    test("Small regular coffee → $3.00", () => {
      expect(calculateCoffeePrice("small", "regular")).toBe(3.0);
    });

    test("Medium regular coffee → $4.00", () => {
      expect(calculateCoffeePrice("medium", "regular")).toBe(4.0);
    });

    test("Large regular coffee → $5.00", () => {
      expect(calculateCoffeePrice("large", "regular")).toBe(5.0);
    });
  });

  describe("Coffee type add-ons", () => {
    test("Small latte → $3.00 + $1.00 = $4.00", () => {
      expect(calculateCoffeePrice("small", "latte")).toBe(4.0);
    });

    test("Medium cappuccino → $4.00 + $1.50 = $5.50", () => {
      expect(calculateCoffeePrice("medium", "cappuccino")).toBe(5.5);
    });

    test("Large mocha → $5.00 + $2.00 = $7.00", () => {
      expect(calculateCoffeePrice("large", "mocha")).toBe(7.0);
    });
  });

  describe("Extras", () => {
    test("Small regular + whipped cream → $3.50", () => {
      expect(calculateCoffeePrice("small", "regular", { whippedCream: true })).toBe(3.5);
    });
    ...
    test("Large mocha + whipped cream + extra shot → $5.00 + $2.00 + $0.50 + $0.75 = $8.25", () => {
      expect(calculateCoffeePrice("large", "mocha", { whippedCream: true, extraShot: true })).toBe(
        8.25,
      );
    });

    test("No extras object passed → just base + type", () => {
      expect(calculateCoffeePrice("small", "mocha")).toBe(5.0);
    });
  });
